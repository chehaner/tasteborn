<template>
  <div class="home-library">
    ...
    <div class="home-library-list">
      <div class="home-library-item" v-for="(item, index) in libraryClass" :key = index>
        <var-tabs
          ...
          indicator-size="0"
        >
          <var-tab v-for="tabName in item" :key = tabName>{{ tabName }}</var-tab>
        </var-tabs>
        <var-tabs-items v-model:active="active[index]" :can-swipe="false">
          <var-tab-item v-for="(itemList, index) in libraryList[index]" :key = index>
            <HomeList
              :id="item.id"
              :pic-url="item.picUrl"
              :name="item.name"
