<template>
  ...
  import { addComment, getComment, getReplyComment, replyComment } from "@/api/blog";
  ...
  async function sendComment() {
    // 进行回复评论和发表评论
    const res = flag.value ? await addComment(props.id, user_id, commentValue.value) : await replyComment(mainCommentId.value, user_id, commentValue.value)
    // 判断用户是否登录
    if (res.status === 401) {
